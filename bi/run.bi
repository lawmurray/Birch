...
program run() {
  ...
  code <- code + system("birch sample --model SIRModel --sampler AliveParticleFilter --config config/sir_model.json --input input/russian_influenza.json --output output/sir_model.json");
  exit(code);
}
