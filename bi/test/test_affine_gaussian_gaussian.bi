 ...
 */
program test_affine_gaussian_gaussian(N:Integer <- 10000) {
  X1:Real[N,2];
  X2:Real[N,2];
  ...
  for i:Integer in 1..N {
    m:TestAffineGaussianGaussian(a, μ_0, σ2_0, c, σ2_1);
    m.initialize();
    X1[i,1..2] <- m.forward();
  }
...
class TestAffineGaussianGaussian(a:Real, μ_0:Real, σ2_0:Real, c:Real, σ2_1:Real) {
  a:Real <- a;
  μ_0:Real <- μ_0;
  σ2_0:Real <- σ2_0;
  c:Real <- c;
