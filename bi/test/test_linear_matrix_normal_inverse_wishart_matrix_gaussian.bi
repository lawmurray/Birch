 ...
 */
program test_linear_matrix_normal_inverse_wishart_matrix_gaussian(
    N:Integer <- 10000) {
  auto n <- 5;
  auto p <- 2;
  ...
  /* simulate forward */
  for auto i in 1..N {
    m:TestLinearMatrixNormalInverseWishartMatrixGaussian(A, M, U, C, k, Ψ);
    m.play();
...
class TestLinearMatrixNormalInverseWishartMatrixGaussian(A:Real[_,_],
    M:Real[_,_], U:Real[_,_], C:Real[_,_], k:Real, Ψ:Real[_,_]) < Model {
  auto A <- A;
  auto M <- M;
  auto U <- U;
  auto C <- C;
