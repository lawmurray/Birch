...
class MultivariateIndependentGaussian(μ:Expression<Real[_]>,
  ...
  function doGraft() -> DelayValue<Real[_]>? {
    ...
    if (s2 <- σ2.graftInverseGamma())? {
      if (m1 <- μ.graftMultivariateAffineNormalInverseGamma(σ2))? {
        return DelayMultivariateAffineNormalInverseGammaGaussian(this, m1!.A, m1!.x, m1!.c);
      ...
      return DelayMultivariateAffineGaussianGaussian(this, m3!.A, m3!.x, m3!.c, diagonal(σ2.value(), m3!.size()));
    } else if (m4 <- μ.graftMultivariateGaussian())? {
      return DelayMultivariateGaussianGaussian(this, m4!, diagonal(σ2.value(), m4!.size()));
    } else {
      μ1:Real[_] <- μ.value();
      return DelayMultivariateGaussian(this, μ1, diagonal(σ2.value(), length(μ1)));
    }
  }
  ...
  function doGraftMultivariateNormalInverseGamma(σ2:Expression<Real>) ->
    ...
    if (s1 <- this.σ2.graftScaledInverseGamma(σ2))? {
      μ1:Real[_] <- μ.value();
      D:Integer <- length(μ1);
      return DelayMultivariateNormalInverseGamma(this, μ1, diagonal(s1!.a2, D), s1!.σ2);
