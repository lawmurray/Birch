...
birch::CppResumeGenerator::CppResumeGenerator(const Class* currentClass,
...
void birch::CppResumeGenerator::visit(const Function* o) {
...
void birch::CppResumeGenerator::visit(const MemberFunction* o) {
  visit(dynamic_cast<const Function*>(o));
}

void birch::CppResumeGenerator::visit(const Yield* o) {
  genTraceLine(o->loc);
  start("yield_");
  genUniqueName(o);
  finish('(' << o->single << ");");
...
void birch::CppResumeGenerator::visit(const Return* o) {
  if (inLambda) {
    CppGenerator::visit(o);
  } else {
    genTraceLine(o->loc);
...
void birch::CppResumeGenerator::visit(const LocalVariable* o) {
...
void birch::CppResumeGenerator::visit(const NamedExpression* o) {
  if (o->isLocal()) {
...
void birch::CppResumeGenerator::genUniqueName(const Numbered* o) {
  if (currentClass) {
...
void birch::CppResumeGenerator::genPackType(const Function* o) {
...
void birch::CppResumeGenerator::genPackParam(const Function* o, const bool move) {
...
void birch::CppResumeGenerator::genPackLocal(const Function* o, const bool move) {
...
void birch::CppResumeGenerator::genUnpackParam(const Function* o) {
  Gatherer<Parameter> params;
  o->params->accept(&params);
...
void birch::CppResumeGenerator::genYieldMacro(const Function* o) {
  auto params = requiresParam(o);
  auto locals = requiresLocal(o);
...
bool birch::CppResumeGenerator::requiresParam(const Function* o) {
  Gatherer<Parameter> params;
  o->accept(&params);
  return params.size() > 0;
}

bool birch::CppResumeGenerator::requiresLocal(const Function* o) {
  Gatherer<LocalVariable> locals([](auto o) { return o->has(RESUME); });
  o->accept(&locals);
  return currentClass || locals.size() > 0;
}

std::string birch::CppResumeGenerator::getName(const std::string& name,
    const int number) {
...
std::string birch::CppResumeGenerator::getIndex(const Statement* o) {
