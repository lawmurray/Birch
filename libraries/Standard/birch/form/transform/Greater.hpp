...
struct GreaterOp {
  ...
  static auto eval(const T& x, const U& y) {
    return numbirch::greater(birch::eval(x), birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad1(G&& g, const T& x, const U& y) {
    return numbirch::greater_grad1(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad2(G&& g, const T& x, const U& y) {
    return numbirch::greater_grad2(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }
...
using Greater = Form<GreaterOp,T,U>;

template<argument T, argument U>
auto greater(T&& x, U&& y) {
  return Greater<tag_t<T>,tag_t<U>>(std::in_place, std::forward<T>(x),
      std::forward<U>(y));
}
