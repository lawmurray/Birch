...
struct DivOp {
  ...
  static auto eval(const T& x, const U& y) {
    return numbirch::div(birch::eval(x), birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad1(G&& g, const T& x, const U& y) {
    return numbirch::div_grad1(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad2(G&& g, const T& x, const U& y) {
    return numbirch::div_grad2(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }
...
using Div = Form<DivOp,T,U>;

template<argument T, argument U>
auto div(T&& x, U&& y) {
  return Div<tag_t<T>,tag_t<U>>(std::in_place, std::forward<T>(x),
      std::forward<U>(y));
}
