...
struct EqualOp {
  ...
  static auto eval(const T& x, const U& y) {
    return numbirch::equal(birch::eval(x), birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad1(G&& g, const T& x, const U& y) {
    return numbirch::equal_grad1(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }

  template<class G, class T, class U>
  static auto grad2(G&& g, const T& x, const U& y) {
    return numbirch::equal_grad2(std::forward<G>(g), birch::eval(x),
        birch::eval(y));
  }
...
using Equal = Form<EqualOp,T,U>;

template<argument T, argument U>
auto equal(T&& x, U&& y) {
  return Equal<tag_t<T>,tag_t<U>>(std::in_place, std::forward<T>(x),
      std::forward<U>(y));
}
