...
class YAMLWriter < Writer {
  ...
  function open(path:String) {
    ...
    cpp{{
    yaml_emitter_initialize(&this_()->emitter);
    yaml_emitter_set_unicode(&this_()->emitter, 1);
    yaml_emitter_set_output_file(&this_()->emitter, this_()->file);
    yaml_stream_start_event_initialize(&this_()->event, YAML_UTF8_ENCODING);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    yaml_document_start_event_initialize(&this_()->event, NULL, NULL, NULL, 1);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    }}
  }
  ...
  function flush() {
    cpp{{
    yaml_emitter_flush(&this_()->emitter);
    }}
    fflush(file);
  }
  ...
  function close() {
    cpp{{
    yaml_document_end_event_initialize(&this_()->event, 1);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    yaml_stream_end_event_initialize(&this_()->event);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    yaml_emitter_delete(&this_()->emitter);
    }}
    fclose(file);
  }
  ...
  function startMapping() {
    cpp{{
    yaml_mapping_start_event_initialize(&this_()->event, NULL, NULL, 1,
  ...
  function endMapping() {
    cpp{{
    yaml_mapping_end_event_initialize(&this_()->event);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    }}
  }
  ...
  function startSequence() {
    cpp{{
    yaml_sequence_start_event_initialize(&this_()->event, NULL, NULL, 1,
  ...
  function endSequence() {
    cpp{{
    yaml_sequence_end_event_initialize(&this_()->event);
    yaml_emitter_emit(&this_()->emitter, &this_()->event);
    }}    
  }
  ...
  function scalar(x:Boolean) {
    auto value <- String(x);
    cpp{{
    yaml_scalar_event_initialize(&this_()->event, NULL, NULL,
