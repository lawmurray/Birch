...
auto transform_pair(const T& x, const U& y, const V& z, Functor f) {
  if constexpr (is_arithmetic_v<T> && is_arithmetic_v<U> &&
    ...
    kernel_transform_pair(m, n, data(x), stride(x), data(y), stride(y),
        data(z), stride(z), data(a), stride(a), data(b), stride(b), f);
    return std::make_pair(a, b);
  }
}
...
auto gather(const T& x, const U& i) {
  ...
  kernel_gather(m, n, data(x), stride(x), data(i), stride(i), data(z),
      stride(z));
  return z;
}
...
auto gather(const T& x, const U& i, const V& j) {
  ...
  auto n = height(i, j);
  auto z = Array<value_t<T>,D>(make_shape<D>(m, n));
  kernel_gather(m, n, data(x), stride(x), data(i), stride(i), data(j),
      stride(j), data(z), stride(z));
  return z;
}
