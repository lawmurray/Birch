...
template<class T, std::enable_if_t<std::is_floating_point<T>::value,int> = 0>
HOST DEVICE T digamma(const T x, const int y) {
  T z = 0.0;
  for (int i = 1; i <= y; ++i) {
...
template<class T, std::enable_if_t<std::is_floating_point<T>::value,int> = 0>
HOST DEVICE T lbeta(const T x, const T y) {
...
template<class T, std::enable_if_t<std::is_floating_point<T>::value,int> = 0>
HOST DEVICE T lgamma(const T x, const int y) {
...
template<class T, std::enable_if_t<std::is_floating_point<T>::value,int> = 0>
HOST DEVICE T pow(const T x, const T y) {
  return std::pow(x, y);
}

template<class T, std::enable_if_t<std::is_floating_point<T>::value,int> = 0>
HOST DEVICE T rcp(const T x) {
  return T(0)/x;
}
